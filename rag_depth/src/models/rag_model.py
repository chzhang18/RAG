...
class Network(nn.Module):
    def __init__(self, genotype, device):
        ...
        self.stem3d0 = nn.ModuleList([ConvBR_2d(initial_fm, initial_fm, 3, stride=1, padding=1)])
        self.stem3d1 = nn.ModuleList([ConvBR_2d(initial_fm, initial_fm, 3, stride=1, padding=1)])
        ...
        for i in range(self._num_layers_3d):
            if i == 0:
                ...
                cell = Cell_2d(self._step, self._block_multiplier, initial_fm / self._block_multiplier, 4, genotype, 4 , 0)

            elif i == 2:
                cell = Cell_2d(self._step, self._block_multiplier, 4, 4, genotype, 4, 0)

            elif i == 3:
                cell = Cell_2d(self._step, self._block_multiplier, 4, 4, genotype, 8, -1)

            elif i == 4:
                cell = Cell_2d(self._step, self._block_multiplier, 4, 8, genotype, 16, -1)

            elif i == 5:
                cell = Cell_2d(self._step, self._block_multiplier, 8, 16, genotype, 8, 1)

            elif i == 6:
                cell = Cell_2d(self._step, self._block_multiplier, 16, 8, genotype, 16, -1)

            elif i == 7:
                cell = Cell_2d(self._step, self._block_multiplier, 8, 16, genotype, 16, 0)
        ...
        self.last_3_3d  = nn.ModuleList([ConvBR_2d(initial_fm, 1, 3, 1, 1,  bn=False, relu=False)]) 
        self.last_6_3d  = nn.ModuleList([ConvBR_2d(initial_fm*2 , initial_fm,    1, 1, 0)])
        self.last_12_3d = nn.ModuleList([ConvBR_2d(initial_fm*4 , initial_fm*2,  1, 1, 0)])


        self.maxdisp = 192
        self.disp = Disp(self.maxdisp)

        self.depth_head = DispHead(input_dim=1)
        self.max_depth = 80
    ...
    def matching(self, x, task_arch, path):
        ...
        upsample_6  = nn.Upsample(size=x.size()[2:], mode='bilinear', align_corners=True)
    ...
    def forward(self, left, right, t, task_arch=None, path=None):
        ...
        depth = self.depth_head(cost, 3)

        disp = torch.squeeze(depth, 1) * self.max_depth
        return disp
    ...
    def expand(self, t, genotype, device='cuda'):
        ...
        self.stem3d0.append(ConvBR_2d(initial_fm, initial_fm, 3, stride=1, padding=1).to(device))
        num_l = self.length['stem_3d0'] + 1
        temp = torch.full((num_l,), 1 / (self._K_multiplier * self.length['stem_3d0'] + 1))
        temp[:num_l-1] *= self._K_multiplier
        self.p.append(temp) # self.p 8

        self.stem3d1.append(ConvBR_2d(initial_fm, initial_fm, 3, stride=1, padding=1).to(device))
        ...
        for i in range(self._num_layers_3d):
            if i == 0:
                ...
                cell = Cell_2d(self._step, self._block_multiplier, initial_fm / self._block_multiplier, 4, genotype, 4 , 0).to(device)

            elif i == 2:
                cell = Cell_2d(self._step, self._block_multiplier, 4, 4, genotype, 4, 0).to(device)

            elif i == 3:
                cell = Cell_2d(self._step, self._block_multiplier, 4, 4, genotype, 8, -1).to(device)

            elif i == 4:
                cell = Cell_2d(self._step, self._block_multiplier, 4, 8, genotype, 16, -1).to(device)

            elif i == 5:
                cell = Cell_2d(self._step, self._block_multiplier, 8, 16, genotype, 8, 1).to(device)

            elif i == 6:
                cell = Cell_2d(self._step, self._block_multiplier, 16, 8, genotype, 16, -1).to(device)

            elif i == 7:
                cell = Cell_2d(self._step, self._block_multiplier, 8, 16, genotype, 16, 0).to(device)
        ...
        self.last_3_3d.append(ConvBR_2d(initial_fm, 1, 3, 1, 1,  bn=False, relu=False).to(device))
        ...
        self.last_6_3d.append(ConvBR_2d(initial_fm*2 , initial_fm,    1, 1, 0).to(device)) 
        ...
        self.last_12_3d.append(ConvBR_2d(initial_fm*4 , initial_fm*2,  1, 1, 0).to(device))
    ...
    def search_matching(self, x, selected_ops, t):
        ...
        stem0 = self.stem3d0[selected_ops[8]](x) # x:[B,12,64,128]
