...
class Cell(nn.Module):
    def __init__(self, steps, block_multiplier, prev_prev_fmultiplier,
        ...
        if downup_sample == -1:
            ...
            self.scale = 2
        for x in self.cell_arch: # self.cell_arch: tensor([[0,1], [1,0], [3,0], [2,0], [8,0], [6,0]])
    ...
    def forward(self, prev_prev_input, prev_input):
        ...
        for i in range(self._steps):
            ...
            for j, h in enumerate(states):
                ...
                if branch_index in self.cell_arch[:, 0]: # [0, 1, 3, 2, 8, 6]
...
class newFeature(nn.Module):
    def __init__(self, network_arch, cell_arch, cell=Cell, args=None):
        ...
        self._step = 3#args.fea_step # 3
        self._num_layers = 4#args.fea_num_layers # 4
        self._block_multiplier = 3#args.fea_block_multiplier # 3
        self._filter_multiplier = 4#args.fea_filter_multiplier # 4

        initial_fm = self._filter_multiplier * self._block_multiplier
        half_initial_fm = initial_fm // 2

        self.stem0 = ConvBR_2d(3, half_initial_fm, 3, stride=1, padding=1)
        self.stem1 = ConvBR_2d(half_initial_fm, initial_fm, 3, stride=3, padding=1)
        self.stem2 = ConvBR_2d(initial_fm, initial_fm, 3, stride=1, padding=1)
        ...
        self.last_3  = ConvBR_2d(initial_fm , initial_fm, 1, 1, 0, bn=False, relu=False) 
        self.last_6  = ConvBR_2d(initial_fm*2 , initial_fm,    1, 1, 0)  
        self.last_12 = ConvBR_2d(initial_fm*4 , initial_fm*2,  1, 1, 0)  
        self.last_24 = ConvBR_2d(initial_fm*8 , initial_fm*4,  1, 1, 0)  
