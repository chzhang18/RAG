...
OPS_3d = {
    'skip_connect_3d': lambda C, stride: Identity_3d() if stride == 1 else FactorizedReduce(C, C),
    '3d_conv_3x3': lambda C, stride: ConvBR_3d(C, C, 3, stride, 1)
}

class NaiveBN_3d(nn.Module):
...
class SepConv_3d(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding):
        super(SepConv_3d, self).__init__()
        self.op = nn.Sequential(
...
class Identity_3d(nn.Module):
    def __init__(self):
        super(Identity_3d, self).__init__()
        self._initialize_weights()
...
class FactorizedReduce_3d(nn.Module):
    def __init__(self, C_in, C_out):
        super(FactorizedReduce_3d, self).__init__()
        assert C_out % 2 == 0
...
class DoubleFactorizedReduce_3d(nn.Module):
    def __init__(self, C_in, C_out):
        super(DoubleFactorizedReduce_3d, self).__init__()
        assert C_out % 2 == 0
        self.relu = nn.ReLU(inplace=False)
...
class FactorizedIncrease_3d(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(FactorizedIncrease_3d, self).__init__()
...
class DoubleFactorizedIncrease_3d(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(DoubleFactorizedIncrease_3d, self).__init__()
