...
# Arguments
parser=argparse.ArgumentParser(description='self-supervised continual stereo')
...
writer = SummaryWriter(log_dir='../self_logs/' + exp_name)
...
for t in range(len(train_data_list)):
    ...
    save_path = "../self_logs/" + exp_name + "/checkpoint_task" + str(t) + ".ckpt"
...
for i in range(d1.shape[0]):
    ...
    for j in range(d1.shape[1]):
        writer.add_text("Results/D1_All", '{:.3f}% '.format(100*d1[i, j]), i)
        print('{:.3f}% '.format(100*d1[i, j]), end='')
    print()
print('*'*100)
print('Done!')
